...
public partial class PdfImportService
{
    ...
    private static bool TryParseTransaction(string line, int year, out string date, out string description, out decimal amount)
    {
        ...
        // Try to match amount at end of line
        ...
        // Remove the amount from the end to get the date + description portion
        var dateDescPortion = trimmed[..amtMatch.Index].TrimEnd();
        if (dateDescPortion.Length == 0) return false;

        // Find date at the start of the remaining text
        ...
        // Normalize the amount sign: CashOut convention: positive = expense, negative = income
        // Amounts in bank statements are typically positive for debits (expenses) and negative or parenthesized for credits (income).
        // The sign from the PDF should match CashOut convention already (positive = outflow/expense),
        // but parenthesized amounts are negative (income) which matches too.
        // Sign should already be correct given how we parsed it above.
